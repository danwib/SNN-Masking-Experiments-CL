...
- **Dynamic Sleep (new):** `python -m scripts.run_experiment --config configs/stage5A1_sequential_X_Xprime_soft_sleep_dynamic.yaml` enables adaptive replay: after a configurable warm-up (fractional passes allowed), each epoch always replays the worst-loss x % of examples and mixes in y % of the remainder using loss-proportional sampling with a probability floor. Losses can be tracked via EMA momentum or last-value updates (config toggle), giving us a lightweight per-sample scheduler without storing logits.
- **Dynamic Sleep + Low Temp:** `python -m scripts.run_experiment --config configs/stage5A1_sequential_X_Xprime_soft_sleep_dynamic_lowtemp.yaml` keeps the adaptive replay schedule but sets `sleep.temperature=0` (internally clamped to 1e‑3), which effectively turns the distillation target into a hard teacher signal. This variant consistently recovers ≥99.8 % on Task 1/2 and ≥99.1 % / 99.9 % on Task 1′/2′ after sleep while needing only 10 adaptive epochs instead of 15 fixed ones thanks to prime-first warm-up, similarity-aware sampling, and per-task probability smoothing.
- **Observation:** As we scale the dataset, we have to extend the sleep phase to keep the consolidation stable. The new dynamic sleep modes let us replace static epoch counts with adaptive replay (always selecting high-loss samples and sprinkling the rest), and the low-temperature variant shows that sharpening the teacher signal can cut sleep iterations without accuracy loss.
